...
plotFun_upset_fTFC12_TDS = function(noFrame = FALSE, noPlot = FALSE) {
  p <- UpSetR::upset(
    data = UpSetR::fromList(all_genes),
...
saveFig(
  file.path(plotDir, 'PR_TDS_fTFC12_genes_overlap'),
  plotFun_upset_fTFC12_TDS,
  width = 4.3,
  height = 3.3,
  res = 500
)
...
allScore_wide = data.table::fread(file.path(
  outDir,
  'TDS_fTFC_scores_wide.csv'
)) %>%
  data.table::setDT()
pPTC_normal_samples = bulk_samples$sampleID[
  bulk_samples$sampleName %in% c('Y24_Normal', 'Y46_Normal')
]
...
plotFun_TDS_score_normal_samples = function(noFrame = FALSE, noPlot = FALSE) {
  ...
  p <- ggplot(
    norm_samples,
    aes(x = source, y = TDS_TotalScore, fill = age_group)
  ) +
    geom_quasirandom(
      width = 0.15,
      alpha = 1,
...
saveFig(
  file.path(plotDir, 'PR_TDS_score_normal_samples_bulk'),
  plotFun_TDS_score_normal_samples,
  rawData = norm_samples,
  width = 4.5,
  height = 5,
  res = 500
)
...
fThy$finalAnn[
  fThy$finalAnn == 'Thyrocytes' & fThy$celltype == 'thy_TH_processing'
] = 'fTFC1'
fThy$finalAnn[
  fThy$finalAnn == 'Thyrocytes' & fThy$celltype == 'thy_Lumen-forming'
] = 'fTFC2'
fThy$annot = fThy$finalAnn
fThy$cellID = rownames(fThy@meta.data)

## Normal adult
aThy_Mosteiro23 = readRDS(
  'Data/published_scRNAseq/Mosteiro_etal_2023/Mosteiro_etal_2023.RDS'
)
aThy_Mosteiro23$annot[
  aThy_Mosteiro23$annot %in% c('aTFC1', 'aTFC2', 'aTFC4', 'aTFC5')
] = 'aTFC1'
  ...
  if (dataset == 'Pu21') {
    ...
    srat = subset(
      ...
      subset = cellID %in%
        srat$cellID[
          srat$celltype %in%
            c('fTFC1', 'fTFC2', 'thy_Lumen-forming') &
    ...
    srat = subset(
      srat,
      subset = cellID %in%
        srat$cellID[srat$celltype %in% c('Epithelial cell', 'Malignant cell')]
    )
    ...
    srat = subset(
      srat,
      subset = cellID %in%
        srat$cellID[srat$annot %in% c('Thyrocytes', 'Tumour')]
    )
    srat$dataset = 'Peng_2021'
  } else if (dataset == 'Zheng25') {
    zheng25_mdat = read.csv(
      '~/FetalThyroidAtlas/Data/published_scRNAseq/Zheng_etal_2025/Zheng25/Zheng25_clean_noMTCells_mdat.csv',
      row.names = 1
    )
    srat@meta.data = cbind(
      srat@meta.data,
      zheng25_mdat[
        match(srat$cellID, rownames(zheng25_mdat)),
        !colnames(zheng25_mdat) %in% colnames(srat@meta.data)
      ]
    )
...
adult_PTC_sratObj_ucell_list = lapply(adult_PTC_sratObj_list, function(x) {
  ucell_score <- run_ucell_score(
    ...
    output_filename = sprintf(
      'TDS_geneSignatures_UCELL_scRNAseq_%s.csv',
      unique(x$dataset)
    ),
    output_dir = file.path(outDir, "ucell_output"),
    overwrite = FALSE,
...
pThy = readRDS(
  'Results/2505/PTC_snRNAseq/03_pThyCancer_annotation/pPTC_clean_soupedXrhoLimNone_annotated_2505.RDS'
)
...
ucell_pPTC$sample = paste0(
  pThy$donor[match(ucell_pPTC$cellID, pThy$cellID)],
...
write.csv(
  ucell_pPTC,
  file.path(outDir, 'TDS_geneSignatures_UCELL_scRNAseq_inhousePaedPTC.csv')
)

## adult PTC
ucell_data_adult = do.call(
  rbind,
  lapply(adult_PTC_sratObj_list, function(x) {
    x@meta.data[, columns]
  })
)

ucell_data_adult$annot_2 = gsub('fTFC1-|fTFC2-', '', ucell_data_adult$annot)
ucell_data_adult$annot_2[
  ucell_data_adult$annot_2 %in% c('Thyrocytes')
] = 'Normal'
ucell_data_adult$annot_2[
  ucell_data_adult$annot_2 %in% c('Malignant cell', 'tumour')
] = 'Tumour'
ucell_data_adult$annot_2[
  ucell_data_adult$annot_2 %in% c('Epithelial cell', 'para-tumour')
] = 'Normal'

write.csv(
  ucell_data_adult,
  file.path(outDir, 'TDS_geneSignatures_UCELL_scRNAseq_adultPTC.csv')
)


## Normal fetal + adult datasets
ucell_data_normal = do.call(
  rbind,
  list(
    'fThy' = fThy@meta.data[, columns],
    'aThy_Mosteiro23' = aThy_Mosteiro23@meta.data[, columns]
  )
)
ucell_data_normal$annot[ucell_data_normal$annot %in% c('aTFC1')] = 'fTFC1'
ucell_data_normal$annot[
  ucell_data_normal$annot %in% c('aTFC2', 'thy_Lumen-forming')
] = 'fTFC2'

write.csv(
  ucell_data_normal,
  file.path(outDir, 'TDS_geneSignatures_UCELL_scRNAseq_fetal.adult.Thy.csv')
)

ucell_data_adult = read.csv(
  file.path(outDir, 'TDS_geneSignatures_UCELL_scRNAseq_adultPTC.csv'),
  row.names = 1
)
ucell_data_normal = read.csv(
  file.path(outDir, 'TDS_geneSignatures_UCELL_scRNAseq_fetal.adult.Thy.csv'),
  row.names = 1
)
...
## Do some plots
celltypes_toKeep = unique(ucell_data$annot[grepl(
  'Thyrocyte|TFC1|TFC2|Tumour|Epithelial|Malignant|Normal',
  ucell_data$annot
)])

dd = ucell_data %>% data.table::setDT()
dd[, `:=`(
  dataset = gsub('_', ' et al., ', dataset),
  annot = ifelse(annot == 'Normal', 'Thyrocytes', annot)
)][,
  `:=`(
    annot = factor(
      annot,
      c('fTFC1', 'fTFC2', 'Thyrocytes', 'Tumour', 'aEndo/Mes')
    ),
    dataset = factor(
      dataset,
      c(
...
saveFig(
  baseName = file.path(plotDir, 'PR_TDS_UCell_scRNAseq_datasets'),
  plotFun = plotFun_TDS_UCell,
  rawData = dd,
  width = 5.5,
  height = 4,
  res = 500
