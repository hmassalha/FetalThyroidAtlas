...
cat(sprintf("\nTDS genes found in gene_map: %d/%d\n", 
            length(found), length(tds_genes_symbols)))
if(length(notfound) > 0) cat("Not found:", paste(notfound, collapse=', '), "\n")

## Check these ensIDs are actually in bulk_ranked
in_matrix <- sum(tds_genes_ens %in% rownames(bulk_ranked))
cat(sprintf("TDS ensIDs present in bulk_ranked: %d/%d\n",
            in_matrix, length(tds_genes_ens)))
...
tds_scores <- simpleScore(bulk_ranked, upSet = tds_genes_ens)

tds_scores$sampleID <- rownames(tds_scores)
tds_scores <- tds_scores %>%
  dplyr::rename(TDS_TotalScore = TotalScore,
...
colnames(deg) <- c('ensID','geneSym','chr','logFC','logCPM','F',
                   'PValue','FDR','pct_fTFC1','pct_fTFC2',
                   'direction','module')
...
overlap_summary <- data.frame(
  ...
  pct_overlap      = round(c(100*length(overlap_fTFC1)/length(fTFC1_genes),
                              100*length(overlap_fTFC2)/length(fTFC2_genes)), 1),
  overlapping_genes = c(paste(overlap_fTFC1, collapse=';'),
                         paste(overlap_fTFC2, collapse=';'))
)
...
sampleID_col <- 'sampleID'   ## <-- CHANGE THIS if needed
...
allScore_merged <- allScore_merged %>%
  dplyr::setNamesgroup_by(source) %>%
  dplyr::setNamesmutate(
    TDS_normalised = TDS_TotalScore - 
      median(TDS_TotalScore[cancerNormal == 'Normal'], na.rm = TRUE)
  ) %>%
  dplyr::setNamesungroup()

## Pivot wide: one row per sample, columns for fTFC1/fTFC2/TDS
allScore_wide <- allScore_merged %>%
  dplyr::select(all_of(sampleID_col), source, cancerType, cancerNormal,
         moduleType, TotalScore, normalised_score, TDS_TotalScore, 
         TDS_normalised) %>%
  tidyr::pivot_wider(
...
## All samples
cor_fTFC1_all <- cor.test(tcga$fTFC1_TotalScore, tcga$TDS_TotalScore, 
                           method = 'spearman')
cor_fTFC2_all <- cor.test(tcga$fTFC2_TotalScore, tcga$TDS_TotalScore, 
                           method = 'spearman')
...
cor_fTFC1_tumour <- cor.test(tcga_t$fTFC1_TotalScore, tcga_t$TDS_TotalScore,
                              method = 'spearman')
cor_fTFC2_tumour <- cor.test(tcga_t$fTFC2_TotalScore, tcga_t$TDS_TotalScore,
                              method = 'spearman')

cat("\n========== Spearman correlations with TDS (TCGA) ==========\n")
cat(sprintf("fTFC1 vs TDS (all):    rho=%.3f, p=%.2e\n",
            cor_fTFC1_all$estimate, cor_fTFC1_all$p.value))
cat(sprintf("fTFC2 vs TDS (all):    rho=%.3f, p=%.2e\n",
            cor_fTFC2_all$estimate, cor_fTFC2_all$p.value))
cat(sprintf("fTFC1 vs TDS (tumour): rho=%.3f, p=%.2e\n",
            cor_fTFC1_tumour$estimate, cor_fTFC1_tumour$p.value))
cat(sprintf("fTFC2 vs TDS (tumour): rho=%.3f, p=%.2e\n",
            cor_fTFC2_tumour$estimate, cor_fTFC2_tumour$p.value))

## Save correlation results
cor_results <- data.frame(
  comparison   = c('fTFC1_vs_TDS_all','fTFC2_vs_TDS_all',
                   'fTFC1_vs_TDS_tumour','fTFC2_vs_TDS_tumour'),
  rho          = c(cor_fTFC1_all$estimate, cor_fTFC2_all$estimate,
                   cor_fTFC1_tumour$estimate, cor_fTFC2_tumour$estimate),
  p_value      = c(cor_fTFC1_all$p.value, cor_fTFC2_all$p.value,
                   cor_fTFC1_tumour$p.value, cor_fTFC2_tumour$p.value),
  n            = c(nrow(tcga), nrow(tcga),
                   nrow(tcga_t), nrow(tcga_t))
)
...
if(nrow(tcga_n) < 20) warning("Normal N is low - regression may be unstable")
...
cat("Q: After accounting for TDS, does fTFC2 still differ between tumour and normal?\n")
cat(sprintf("Wilcoxon W=%.0f, p=%.2e\n", 
            resid_test$statistic, resid_test$p.value))
cat(sprintf("Median residual - Normal: %.4f\n",
            median(tcga$fTFC2_residual[tcga$cancerNormal == 'Normal'])))
cat(sprintf("Median residual - Tumour: %.4f\n",
...
plotFun_TDS_scatter <- function(noFrame=FALSE, noPlot=FALSE){
  ...
  p <- ggplot(dd, aes(TDS_TotalScore, fTFC2_TotalScore, 
                       col = cancerNormal)) +
    ...
    xlab('TDS score') + ylab('fTFC2 score')
  
  print(p)
}
saveFig(file.path(plotDir, 'Fig_TDS_fTFC2_scatter'),
        plotFun_TDS_scatter,
        rawData = tcga,
        width = 3.5, height = 3.5, res = 500)
...
plotFun_TDS_residual <- function(noFrame=FALSE, noPlot=FALSE){
  ...
  plab <- ifelse(resid_test$p.value < 0.001,
  ...
  p <- ggplot(dd, aes(cancerNormal, fTFC2_residual)) +
    ...
    geom_boxplot(aes(fill = cancerNormal),
                 outlier.shape = NA,
                 alpha = 0.8, width = 0.4,
...
saveFig(file.path(plotDir, 'Fig_TDS_fTFC2_residual'),
        plotFun_TDS_residual,
        rawData = tcga,
        width = 2.2, height = 3.2, res = 500)
...
saveFig(file.path(plotDir, 'Fig_TDS_gene_overlap'),
        plotFun_overlap,
        rawData = overlap_summary,
        width = 2.5, height = 3, res = 500)


##--- Step 9: Summary printout ---##
cat("\n\n========== TDS ANALYSIS SUMMARY ==========\n")
cat("\nGene overlap:\n")
print(overlap_summary[, c('signature','n_total_genes',
                           'n_TDS_overlap','pct_overlap')])
cat("\nSpearman correlations with TDS (TCGA):\n")
print(cor_results)
cat("\nPartial correlation (fTFC2 residual after TDS):\n")
cat(sprintf("  p = %.2e  -->  %s\n",
            resid_test$p.value,
            ifelse(resid_test$p.value < 0.05,
